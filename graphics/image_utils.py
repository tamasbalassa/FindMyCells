...
def drawRect(pixmap, rectangles):   
        qp = QPainter(pixmap)
        xScale = pixmap.width() / float(FMC_settings.gInputWidth)
        yScale = pixmap.height() / float(FMC_settings.gInputHeight)
...
def set_image(self, imgpath, mode='Single'):                
        ...
        pixmap_resized = pixmap.scaled(FMC_settings.gWidth, FMC_settings.gHeight, Qt.KeepAspectRatio)
