...
def gridbox_to_boxes(net_cvg, net_boxes, self):
    ...
    x1 = (np.asarray([net_boxes[0][y[i]][x[i]] for i in range(x.size)]) + mx)
    y1 = (np.asarray([net_boxes[1][y[i]][x[i]] for i in range(x.size)]) + my)
    x2 = (np.asarray([net_boxes[2][y[i]][x[i]] for i in range(x.size)]) + mx)
    y2 = (np.asarray([net_boxes[3][y[i]][x[i]] for i in range(x.size)]) + my)

    boxes = np.transpose(np.vstack((x1, y1, x2, y2)))
    cvgs = np.transpose(np.vstack((x, y, np.asarray(
        [cvg_val[y[i]][x[i]] for i in range(x.size)]))))
